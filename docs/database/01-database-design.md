...
本项目采用 MySQL 8.0 作为关系型数据库，使用 SQLAlchemy 2.0 Async 完成异步数据访问，并通过 Alembic 管理数据库结构迁移。数据库模型由初始 12 张核心表加认证会话扩展表组成，覆盖用户权限、项目管理、研发协作和消息通知四个业务领域。
...
DevFlow 当前版本包含 13 张核心数据表，按业务领域划分如下：

| 业务领域 | 数据表 | 主要职责 |
| --- | --- | --- |
| 用户权限领域 | `users` | 保存系统用户基础信息 |
| 用户权限领域 | `auth_sessions` | 保存 Refresh Token 轮换与设备会话状态 |
...
- `version` 通过条件更新防止并发请求静默覆盖其他请求的修改；
...
### 5.13 auth_sessions 认证会话表

`auth_sessions` 保存 Access Token 绑定的设备会话和当前 Refresh Token 摘要，用于轮换、重放检测与主动注销。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | VARCHAR(36) | 会话 UUID 主键 |
| `user_id` | BIGINT | 所属用户外键 |
| `refresh_token_hash` | VARCHAR(64) | 当前 Refresh Token SHA-256 摘要，唯一 |
| `rotation_count` | INTEGER | 轮换次数 |
| `ip_address` | VARCHAR(45) | 登录 IP |
| `user_agent` | VARCHAR(255) | 登录客户端 |
| `expires_at` | DATETIME | 会话过期时间 |
| `last_used_at` | DATETIME | 最后轮换时间 |
| `revoked_at` | DATETIME | 主动撤销时间 |
| `created_at` | DATETIME | 创建时间 |

数据库不保存原始 Refresh Token。受保护请求除校验 JWT 签名和过期时间外，还检查会话未撤销、未过期。
...
| `auth_sessions` | `refresh_token_hash` | 唯一索引 | 防止摘要重复并定位当前令牌 |
| `auth_sessions` | `user_id` | 普通索引 | 支持全设备注销 |
...
初始 12 张核心表使用 BIGINT 主键，认证会话表使用不可预测的 UUID 字符串主键。主键用于唯一标识记录，并作为实体关联的稳定引用目标。
...
- `notifications.user_id` 关联 `users.id`。
- `auth_sessions.user_id` 关联 `users.id`。
...
上述内容仅作为未来扩展方向，当前数据库模型包含 13 张核心表，不包含 `tags`、`attachments` 和 `activity_logs`。
