...
3. `app/models/` 与 `alembic/versions/`：13 张表如何映射和演进；
...
→ decode_access_token
→ 校验 AuthSession 未过期、未撤销
→ 加载未删除且启用的 User
→ CurrentUser
```

Token 只保存用户 ID、会话 ID、令牌 ID、签发时间、过期时间和类型，不保存密码或权限快照。Refresh Token 每次使用后轮换，数据库只保存 SHA-256 摘要。
...
测试分为健康检查、认证/RBAC、项目、Issue、评论/Review、通知、Prometheus 指标和迁移版本，并包含真实 MySQL 并发请求。
...
| 认证 | `security.py`、`auth_session.py`、`auth_service.py`、`dependencies.py` |
...
| 可观测性 | `logging.py`、`metrics.py`、`scripts/run_load_test.py` |
| 部署 | `Dockerfile`、两个 Compose、`nginx.conf`、`prometheus.yml` |

## 19. 每阶段应该能回答的问题

- 配置：敏感信息从哪里来，为什么不会提交？
- 数据库：一个请求使用几个 Session，事务由谁提交？
- 认证：密码为何无法恢复，Refresh Token 如何轮换和撤销？
- RBAC：系统角色和项目角色为什么不能合并？
- 项目：创建项目失败时 Owner 关系会怎样？
- Issue：为什么不能从 OPEN 直接跳 DONE，乐观锁如何避免丢失更新？
- Review：如何保证 Review 与 Issue 一致，并让重复审批幂等？
- 通知：Celery 停止时用户是否还能看到通知？
- 测试：为什么测试库不会污染开发库？
- Docker：MySQL 未健康时 API 会不会提前启动？
- 指标：为什么 Prometheus 路由标签不能使用实际 Issue ID？
